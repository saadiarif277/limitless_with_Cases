    ...
    public function index(Request $request): Response
{
    ...
    if ($currentUser->hasRole('Doctor')) {
        // If the user is a doctor, only fetch cases where the piloting_physician_id matches the authenticated user's ID
        $casesQuery->where('cases.piloting_physician_id', $currentUser->user_id);
    } elseif ($currentUser->hasRole('Patient')) {
        // If the user is a patient, only fetch cases where the patient_id matches the authenticated user's ID
        $casesQuery->where('cases.patient_id', $currentUser->user_id);
    } elseif ($currentUser->hasRole('Attorney')) {
        // If the user is an attorney, only fetch cases where the attorney_id matches the authenticated user's ID
        $casesQuery->where('cases.attorney_id', $currentUser->user_id);
    } // Add more role-based conditions as needed

    // Fetch the filtered cases

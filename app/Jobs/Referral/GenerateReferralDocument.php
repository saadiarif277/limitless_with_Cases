...
class GenerateReferralDocument implements ShouldQueue
{
    ...
    {
        $documentType = DocumentType::findOrFail(DocumentType::REFERRAL_SUMMARY);
        ...
        $html = view('pdf.referral', ['referral' => $this->referral])->render();
        ...
        $referralDocument = $this->referral
            ->documents()
            ->where('document_type_id', DocumentType::REFERRAL_SUMMARY)
            ->first();

        if (!$referralDocument) {
            $this->referral->documents()->create([
                'document_type_id' => DocumentType::REFERRAL_SUMMARY,
                'name' => $fileName,
                'path' => $path,
            ]);
        } else {
            $referralDocument->update([
                'name' => $fileName,
                'path' => $path,
            ]);
        }

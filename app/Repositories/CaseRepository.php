...
class CaseRepository
{
    ...
    //         // Prepare the payload for the case
    ...
    //         // Ensure that the referral will always have a source user
    //         if (!$request->filled('source_user_id')) {
    //             $payload['source_user_id'] = $currentUser ? $currentUser->getKey() : User::SYSTEM;
    //         }

    //         // Map doctor (piloting_physician_id)
    //         $doctorUser = $this->getDoctorUser($request);
    //         $payload['piloting_physician_id'] = $doctorUser->getKey();

    //         // Map attorney
    //         $attorneyUser = $this->getAttorneyUser($request);
    //         $payload['attorney_id'] = $attorneyUser->getKey();

    //         // Map patient
    //         $patientUser = $this->getPatientUser($request);
    //         $payload['patient_id'] = $patientUser->getKey();

    //         // Map policy limit info
    //         $payload['policy_limit_info'] = $request->input('insuranceData');

    //         // Map ICD-10 codes
    //         $payload['icd10_codes'] = $request->input('ict_code_id'); // Assuming it's a single ICD-10 code; adjust as needed

    //         // Map CPT codes
    //         $payload['cpt_codes'] = implode(',', $request->input('selectedCptCodes', '')); // Convert array to comma-separated string

    //         // Additional static or derived fields
    //         $payload['service_billed'] = $request->input('service_billed', 0); // Default to 0 if not provided
    //         $payload['billing_type'] = $request->input('billing_type', ''); // Default empty if not provided
    //         $payload['is_cms1500_generated'] = false; // Default to false
    //         $payload['case_won'] = null; // Default to null (not determined yet)
    //         $payload['outcome'] = null; // Default to null
    //         $payload['reduction_accepted'] = null; // Default to null
    //         $payload['is_closed'] = false; // Default to false
    //         $payload['closed_at'] = null; // Default to null

    //         // Create the case
    //         $case = Cases::create($payload);

    //         // Attach referral reasons if provided
    //         $referralReasonIds = $request->input('patient.referral_reason_ids', []);
    //         if (!empty($referralReasonIds)) {
    //             $case->referralReasons()->sync($referralReasonIds);
    //         }

    //         // Create documents if provided
    //         $documents = $request->input('documents', []);
    //         if (!empty($documents)) {
    //             $this->createDocuments($documents, $case);
    //         }
    ...
    public function create(Request $request)
{
    ...
    try {
        ...
        Log::info('Incoming request for case creation:', [
            'request_data' => $request->all(),
            'current_user' => $currentUser,
        ]);
        // Ensure that the referral will always have a source user
        $payload['source_user_id'] = $request->input('source_user_id', $currentUser ? $currentUser->getKey() : User::SYSTEM);
        ...
        $payload['policy_limit_info'] = json_encode($request->input('insuranceData'));

        // ICD-10 codes (integer, no need for implode)
        $payload['icd10_codes'] = $request->input('ict_code_id');

        // CPT codes (convert array to comma-separated string)
        $cptCodes = $request->input('selectedCptCodes', []);
        $payload['cpt_codes'] = implode(',', $cptCodes);
        ...
        $payload['is_closed'] = false; // Default to false
        $payload['closed_at'] = null; // Default to null
        $payload['primary_referral_id']=$request->input('primary_referral_id');

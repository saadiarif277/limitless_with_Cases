    ...
    <div class="divide-y divide-gray-200">
        ...
        <v-content-body class="relative grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-6 gap-4">
            ...
            <v-form-group class="col-span-full">
                <v-form-label>Referral Reasons</v-form-label>
                <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3">
                    <template v-for="(referralReason, referralReasonIndex) in referralReasons.data" :key="'referral_reason_' + referralReasonIndex">
                        <v-form-checkbox v-model="form.patient.referral_reason_ids" :value="referralReason.referral_reason_id" :disabled="form.processing">
                            {{ referralReason.name }}
                        </v-form-checkbox>
                    </template>
                </div>
                <v-form-error v-if="form.errors['patient_referral_reason_ids']">{{ form.errors['patient_referral_reason_ids'] }}</v-form-error>
            </v-form-group>
        </v-content-body>
    </div>
</template>

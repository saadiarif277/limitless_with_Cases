    ...
    <div class="h-full flex flex-col">
            ...
            <v-section-heading>
                <template #title>
                    <template v-if="!appointment">
                        ...
                        <template v-else>
                            <span>
                                for the referral <v-link :href="route('panel.admin.referrals.show', { referral: form.referral_id })">"REF#{{ form.referral_id }}"</v-link>
                            </span>
                        </template>
                    </template>
                ...
                <template #actions>
                    <template v-if="appointment">
                        <div class="flex gap-2">
                            <v-button :href="route('panel.admin.referrals.show', { referral: form.referral_id })" :disabled="form.processing" color="white" v-if="form.referral_id">
                                View Referral
                            </v-button>
        ...
        <v-content-body>
            <template v-if="!appointment && !form.referral_id">
                <v-paragraph>
                    <span class="text-red-500 font-bold">Error: Referral not found.</span> <br /> Please create an appointment via a referral <v-link :href="route('panel.admin.referrals.index')">here.</v-link>
                </v-paragraph>
            </template>
            ...
            <template v-else>
                <v-form class="grid grid-cols-1 sm:grid-cols-6 gap-6" @submit.prevent="submitForm">
                    ...
                    <v-form-group class="col-span-full sm:col-span-3">
                        <v-form-label>
                            ...
                            <span class="italic" v-if="form.referral_id">
                                (Inherited from <v-link :href="route('panel.admin.referrals.show', { referral: form.referral_id })">"REF#{{ form.referral_id }}"</v-link>)
                            </span>
                        </v-form-label>
                    ...
                    <v-form-group class="col-span-full flex items-center justify-end gap-2 text-right border-t border-gray-200 py-6">
                        ...
                        <v-button :href="route('panel.admin.appointments.index')" color="white" :disabled="form.processing">
                            Cancel
                        </v-button>
...
export default {
    ...
    methods: {
        ...
        storeRequest() {
            this.form.post(this.route('panel.admin.appointments.store'), {
        ...
        updateRequest() {
            this.form.patch(this.route('panel.admin.appointments.update', { appointment: this.appointment.data.appointment_id }), {
        ...
        deleteRequest() {
            this.$alert().confirm(() => {
                return new Promise((resolve, reject) => {
                    this.form.delete(this.route('panel.admin.appointments.destroy', { appointment: this.appointment.data.appointment_id }), {
                        onSuccess: () => {

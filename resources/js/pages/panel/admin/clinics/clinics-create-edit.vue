<template>
    <v-inertia-head :title="(!clinic) ? 'Create Clinic' : `Editing '${clinic.data.name}' clinic.`" />
        ...
        <v-content-body class="border-b border-gray-200">
            <v-section-heading>
                <template #title>
                    ...
                    <template v-else>
                        Editing '{{ clinic.data.name }}' clinic.
                    </template>
                </template>

                <template #description>
                    <template v-if="!clinic">
                        Fill up the form below to create a new clinic for the application.
                    </template>

                    <template v-else>
                        Change and/or modify the details for the clinic.
                    </template>
                </template>
        ...
        <v-content-body>
            <v-form class="grid grid-cols-1 sm:grid-cols-6 gap-6" @submit.prevent="submitForm">
                ...
                <v-form-group class="col-span-full sm:col-span-3">
                    <v-form-label>Scan Price</v-form-label>
                    <v-form-input type="text" v-model="form.price_scan" :error="form.errors.price_scan" :disabled="form.processing" />
                    <v-form-error v-if="form.errors.price_scan">{{ form.errors.price_scan }}</v-form-error>
                </v-form-group>

                <v-form-group class="col-span-full sm:col-span-3">
                    <v-form-label>Read Price</v-form-label>
                    <v-form-input type="text" v-model="form.price_read" :error="form.errors.price_read" :disabled="form.processing" />
                    <v-form-error v-if="form.errors.price_read">{{ form.errors.price_read }}</v-form-error>
                </v-form-group>
                ...
                <v-form-group class="col-span-full flex items-center justify-end gap-2 text-right border-t border-gray-200 py-6">
                    ...
                    <v-button :href="route('panel.admin.clinics.index')" color="white" :disabled="form.processing">
                        Cancel
                    </v-button>
...
export default {
    ...
    methods: {
        submitForm() {
            ...
            if (this.clinic && this.clinic.data && this.clinic.data.clinic_id) {
                this.updateRequest();
            } else {
                this.storeRequest();
            }
        ...
        storeRequest() {
            this.form.post(this.route('panel.admin.clinics.store'), {
                onSuccess: () => this.$toast().success("Clinic created successfully!"),
                onError: (errors) => {
        ...
        updateRequest() {
            this.form.patch(this.route('panel.admin.clinics.update', { clinic: this.clinic.data.clinic_id }), {
                onSuccess: () => this.$toast().success("Clinic updated successfully!"),
                onError: (errors) => {
                    Object.keys(errors).forEach((key) => {
                        const error = errors[key];
                        this.$toast().error(error);
        ...
        deleteRequest() {
            this.$alert().confirm(() => {
                return new Promise((resolve, reject) => {
                    this.form.delete(this.route('panel.admin.clinics.destroy', { clinic: this.clinic.data.clinic_id }), {
                        onSuccess: () => {

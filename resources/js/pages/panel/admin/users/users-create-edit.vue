                ...
                <template #actions>
                    <template v-if="user">
                        <div class="flex gap-2">
                            <!--
                                <v-button color="white" :href="route('panel.admin.referrals.create', { user_id: user.data.user_id })">
                                    Create Referral
                                </v-button>
                            -->
            ...
            <v-form class="grid grid-cols-1 md:grid-cols-2 gap-6" @submit.prevent="submitForm">
                ...
                <v-card class="col-span-full">
                    <v-content-body class="col-span-full grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-x-6 gap-y-4">
                        ...
                        <div class="col-span-full grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-x-6 gap-y-4">
                            <v-form-group>
                                <v-form-label>Law Firm</v-form-label>
                                <v-form-select
                                    :options="lawFirms.data.map((lawFirm) => ({ label: lawFirm.name, value: lawFirm.law_firm_id }))"
                                    :error="form.errors.law_firm_id"
                                    :disabled="form.processing"
                                    v-model="form.law_firm_id"
                                />
    ...
    watch: {
        user: {
            handler(value) {
                ...
                this.form.law_firm_id = value.data.law_firm_id;
